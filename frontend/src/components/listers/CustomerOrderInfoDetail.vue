<template>
    <v-card outlined>
        <v-card-title>
            CustomerOrderInfo # {{item._links.self.href.split("/")[item._links.self.href.split("/").length - 1]}}
        </v-card-title>
        ...
        <v-card-text>
            ...
            <StringManager offline label="Menu" v-model="item.menu" :editMode="false" @change="change" />
        </v-card-text>
...
<script>
    ...
    export default {
        ...
        async created() {
            ...
            var temp = await axios.get(axios.fixUrl('/customerOrderInfos/' + params.id))
            if(temp.data) {
                me.item = temp.data
            }
        },
